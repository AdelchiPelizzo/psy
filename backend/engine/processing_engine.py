    ...
    def generate_question(self, user_input, strategy="evidence", depth=2, mode="neutral", lang="en"):
        ...
        if lang == "it":
            system_prompt = (
                ...
                "Poi poni UNA domanda mirata per raccogliere informazioni utili ad un'analisi più profonda."                
                "Regole:"
                "- NON porre domande a meno che l'affermazione sia impossibile da interpretare."
                "- La domanda deve essere specifica e utile."
                "- Evidenzia emozioni, schemi comportamentali o motivazioni sottostanti."
                "- Concentrati su schemi ricorrenti, trigger, relazioni o situazioni tipiche."
                ...
                "Domanda: <una domanda mirata>"
            )
        else:
            system_prompt = (
                "You are a psychoanalytic AI assistant."
                "Task:"
                " Analyze the user's statement and provide a concise psychological insight. Then ask ONE targeted follow-up question that helps gather deeper context for future analysis."
                "Rules:"
                "- The question must be specific and purposeful."
                "- Do not ask generic questions (e.g., 'How does that make you feel?')."
                "- Do NOT ask questions unless the statement is impossible to interpret."
                "- Provide an observation that reveals underlying emotions, patterns, triggers, relationships, or recurring situations or motivations."
                ...
                "Question: <one targeted question to deepen understanding>"
            )
